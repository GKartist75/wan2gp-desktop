...
> **New in v3.0.2** — **RTX 40/50 SageAttention fix + RTX 3080 `accelerate` fix + visible install progress.** RTX 40/50 users were on an upstream SageAttention wheel (`2.2.0+cu130torch2.9.0andhigher`) whose fp8 kernel corrupts the CUDA context under torch 2.10 — causing false out-of-memory errors, GPU hangs, and black MiniMax H3. v3.0.2 **auto-swaps it to the stable `cu128torch2.7.1` build** on RTX 40/50 (after install / update / Kernel sync), keeping full SageAttention speed. RTX 3080 installs no longer skip the z-image VAE fix: the bootstrap is deferred so a missing `accelerate` can't abort it, and `ensureAccelerate()` installs it when absent. The installer also now shows a **15s activity heartbeat** during silent setup stretches so it never looks frozen, and a red **SAGE** banner tells affected RTX 40/50 users to click **Sync Kernels**. See [CHANGELOG-v3.0.2.md](changelogs/CHANGELOG-v3.0.2.md).
