...
- **v2.8.6** — **Blank-screen harden (two more classes solved).** **(A) Presentation class** — on some GPUs (e.g. RTX 3060, and even swiftshader / d3d11-direct-composition-off) `ready-to-show` fired and the frame painted once, then the compositor dropped the layer and the window went black with no `first-paint` (issue #45, Marco). A present-hammer now keeps re-committing the frame (`invalidate()` + focus + a 1px resize nudge every 500 ms, capped 15 s, stops on a real paint), and the 8 s watchdog calls it too and reports "first frame never committed" (the old message wrongly said "no ready-to-show"). **(B) Stale data-dir override** — the launcher pins its data dir once to `%USERPROFILE%\.wan2gp-desktop-data-dir` and never re-validated it; renaming/moving the Wan2GP folder then reinstalling left a dead pin so `getRepoDir()` resolved to a void and the launcher blanked (rename → blank, restore → works determinism, reported by JedsDeadBaby). `getDataDir()` and the startup redirect now verify the pinned dir (or its `Wan2GP` core) still exists; if stale, the override is dropped and the default is re-derived and re-pinned, so a renamed/moved folder self-heals on next launch. See [CHANGELOG-v2.8.6.md](changelogs/CHANGELOG-v2.8.6.md).
