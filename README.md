...
> **New in v2.8.7** — **Blank-screen harden (the `show:false` never-presents class).** The v2.8.6 present-hammer (`invalidate()` + resize nudge) worked on the dev machine but **failed for multiple reporters** (RTX 3060, RTX 5070 Ti, driver 610.88) — a CDP probe showed the renderer alive at 60fps yet the Chromium content layer never committed a frame (screenshot returned only the html background). Root cause: a `BrowserWindow` created with `show:false` doesn't always present its first frame on those GPU/compositor stacks. Fix: `paintWhenInitiallyHidden:true` now rasterizes the content **while hidden**, so `show()` presents an already-committed frame. The v2.8.6 hammer + 8s watchdog remain as backstops.
> [Full changelog →](changelogs/CHANGELOG-v2.8.7.md)

> **New in v2.8.6** — **Presentation-class blank screen, attempted.** v2.8.5 still blanked on some GPUs — `ready-to-show` fired but the frame never committed. v2.8.6 added a **present hammer** (`focus()` + `invalidate()` + a 1px resize nudge every 500ms) to force the compositor to re-present. *Correction:* this hammer was sufficient on the dev machine but **did not fix** the affected reporters' hardware (see v2.8.7). The separate **stale data-dir override** self-heal in v2.8.6 is still valid.
...
- **v2.8.7** — **Blank-screen harden (the `show:false` never-presents class).** The v2.8.6 present-hammer (`invalidate()` + resize nudge) worked on the dev machine but **failed for multiple reporters** (RTX 3060, RTX 5070 Ti, driver 610.88): a CDP probe showed the renderer alive at 60fps yet `Page.captureScreenshot` returned only the html background — the Chromium content layer never committed a frame, regardless of GPU backend (native / swiftshader / d3d11). Root cause: a `BrowserWindow` created with `show:false` does not always present its first frame on those stacks. Fix: `paintWhenInitiallyHidden:true` now rasterizes the content **while the window is hidden**, so `show()` presents an already-committed frame. The v2.8.6 hammer + 8s watchdog remain as backstops. (The v2.8.6 changelog's "painted once then reverted" wording was wrong — the frame never committed on the affected hardware; corrected here.) See [CHANGELOG-v2.8.7.md](changelogs/CHANGELOG-v2.8.7.md).
- **v2.8.6** — **Blank-screen harden (two more classes attempted/solved).** **(A) Presentation class** — v2.8.6 added a present-hammer (`invalidate()` + focus + a 1px resize nudge every 500 ms, capped 15 s) to force the compositor to re-present the first frame. *Correction:* this hammer was sufficient on the dev machine but **did not fix** the affected reporters' hardware (those are solved by v2.8.7's `paintWhenInitiallyHidden`). **(B) Stale data-dir override** (still valid) — the launcher pins its data dir once and never re-validated it; renaming/moving the Wan2GP folder then reinstalling left a dead pin so the launcher blanked. Now self-heals: the override is dropped and the default re-derived if stale. See [CHANGELOG-v2.8.6.md](changelogs/CHANGELOG-v2.8.6.md).
