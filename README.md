...
The **Manage → Settings** tab also holds the **GitHub token** field (used to lift
the GitHub API rate limit for update checks — see *Auto-update & the GitHub token*
below) and the **Desktop → *Auto-update*** toggle (turn off launch-time update
checks / silent downloads / install-on-quit).
...
> **New in v3.0.4** — **Fixes the v3.0.3 swap wheel's import crash (RTX 40/50).** v3.0.3 installed a **CUDA-12.8** SageAttention wheel that failed at runtime in the launcher's **CUDA-13.0 / torch 2.10** env (`ImportError: DLL load failed while importing _fused` — the `cu128` `_fused.pyd` links CUDA 12.8 runtimes that aren't present under cu130). The swap now targets the **CUDA-13.0-native, fp8-fixed** `sageattention-2.2.0+cu130torch2.10.0andhigher.post6-cp310-abi3` wheel (abi3 → installs on Python 3.11, same CUDA stack as the env, fp8 out-of-bounds bug fixed per SageAttention #98). Bumped to 3.0.4 so the **auto-updater offers it to 3.0.3 users** too. See [CHANGELOG-v3.0.4.md](changelogs/CHANGELOG-v3.0.4.md).

> **New in v3.0.5** — **The v3.0.4 wheel target was correct, but the self-heal only ran on install / sync / update — not on plain Launch.** So a user who updated to 3.0.4 and just hit Launch (never synced) kept the broken `cu130torch2.9.0andhigher` wheel and crashed with the *same* `_fused` DLL error. v3.0.5 now runs the SageAttention self-heal **and** the `accelerate` check inside the **Launch** handler too (both idempotent). **Updating to 3.0.5 alone fixes the wheel on next Launch — no manual Sync Kernels needed.** See [CHANGELOG-v3.0.5.md](changelogs/CHANGELOG-v3.0.5.md).
...
> **New in v3.0.8** — **Cross-drive install cleanup hardened (#76 / #73).** The `moveDirAtomic` helper already fixed the *abort* in 3.0.7 (rename → copy on `EXDEV`), but its post-copy cleanup gated `rmSync(src)` on `readdirSync(src)`, which **throws on a plain file** and could leak the source file. v3.0.8 removes the source unconditionally after a successful copy, so installing to a drive other than `%TEMP%` leaves a clean temp clone with no readdir error. Regression test added. See [CHANGELOG-v3.0.8.md](changelogs/CHANGELOG-v3.0.8.md).
...
> routine Wan2GP-core poll stays well under the unauthenticated GitHub API rate
> limit. (The **launcher self-update** check is a separate, uncached call — see
> *Auto-update & the GitHub token* below for the rate-limit note and token fix.)
...
The launcher checks **GitHub Releases** for a newer build. Two independent
checks run (each is a separate GitHub API call):
...
> **⚠️ GitHub API rate limit.** Both calls are **unauthenticated** by default,
> and GitHub caps anonymous API access at **60 requests/hour per IP**. If you
> restart the launcher often (e.g. while testing) or share an IP with others,
> that bucket can empty and you'll see *"GitHub rate limited — add a token in
> Manage settings"*. The check then fails until the window resets.
>
> **Fix — add a GitHub token (your own, per-user):** open **Manage → Settings →
> *GitHub token***, paste a **classic personal access token** with the **`repo`**
> (or at least `public_repo`) scope, and **Save**, then **restart** the launcher.
> This raises the limit to **5000 requests/hour** and is drawn against **your**
> token only — it is stored in your local `desktop-config.json` and is **never
> shipped in the `.exe`**, so it can't be seen by or shared with other users.
> Generate one at GitHub → *Settings → Developer settings → Personal access
> tokens → Tokens (classic)*.

> **Note:** the Wan2GP-core poll is cached 5 minutes, so normal day-to-day use
> won't exhaust the anonymous limit — the rate-limit message almost always comes
> from repeated launcher restarts or a busy shared IP, and is resolved by adding
> the token above.
