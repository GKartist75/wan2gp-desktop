...
- **[v3.1.0](changelogs/CHANGELOG-v3.1.0.md)** — Guided LLM engine setup for Deepy Prime. Data-driven engine cards (Claude Code / OpenAI Codex / OpenCode) from `services/llm-engines.js` with one-click install + OpenCode server toggle; the name-whitelist is replaced by a safe pip-spec validator (any PEP 440 pin, no shell metachars); the Advanced pip box gained a copyable command preview and accepts a full pasted `pip install ...` line; Windows `.cmd` resolution fixed the `spawn npm ENOENT` bug. **Cost reality:** OpenCode is free (local models); Claude Code needs a Max/Pro plan *or* a paid Console API key (credits required — not free); Codex needs OpenAI access. 157 tests pass.
