...
- **[v3.1.0](changelogs/CHANGELOG-v3.1.0.md)** — Guided LLM engine setup for Deepy Prime + full Deepy activation panel. Data-driven engine cards (Claude Code / OpenAI Codex / OpenCode) from `services/llm-engines.js` with one-click install + OpenCode server toggle; the name-whitelist is replaced by a safe pip-spec validator (any PEP 440 pin, no shell metachars); the Advanced pip box gained a copyable command preview and accepts a full pasted `pip install ...` line; Windows `.cmd` resolution fixed the `spawn npm ENOENT` bug. **Deepy panel** adds a Disabled / Zero / Prime mode switch with a live-re-rendering **Local model (Prompt Enhancer)** selector (shown for Disabled/Zero, hidden for Prime); Apply writes a consistent `wgp_config.json` (`enhancer_enabled` + `llm_engines.deepy` + full preset) — fixing the root cause where the selected local model was ignored on launch. OpenCode now stores the literal `"opencode"` executable (Wan2GP auto-detects). 173 tests pass.
