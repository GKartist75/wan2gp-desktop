...
- **[v3.1.1](changelogs/CHANGELOG-v3.1.1.md)** — Fix: the guided **OpenCode / Codex** "Install via npm" button (and OpenCode **Start server**) failed on machines where Node lives in a path with a space (the default `C:\Program Files\nodejs`), erroring `'C:\Program' is not recognized`. Root cause: the command ran through a shell with the resolved npm path passed unquoted. Now spawns the resolved shim directly with an argv array (no shell). 176 tests pass.
