...
async function getDirSizeAsync(dir) {
  ...
  try {
    ...
    for (const e of entries) {
      const p = path.join(dir, e.name)
      if (e.isDirectory()) total += await getDirSizeAsync(p)
...
function mergeDirContents(src, dst, onProgress) {
  try {
    ...
    for (const name of items) {
      ...
      if (fs.existsSync(d)) {
        // Directory at both sides → merge recursively instead of skipping whole tree
        try {
          const sSt = fs.statSync(s); const dSt = fs.statSync(d)
          if (sSt.isDirectory() && dSt.isDirectory()) { mergeDirContents(s, d, onProgress); try { if (fs.readdirSync(s).length === 0) fs.rmSync(s, { recursive: true, force: true }) } catch {} continue }
        } catch {}
        continue // don't clobber file/symlink
      }
...
async function mergeDirContentsAsync(src, dst, onProgress) {
  try {
    ...
    for (const name of items) {
      ...
      if (fs.existsSync(d)) {
        try {
          const sSt = await fsp.stat(s); const dSt = await fsp.stat(d)
          if (sSt.isDirectory() && dSt.isDirectory()) { await mergeDirContentsAsync(s, d, onProgress); try { if ((await fsp.readdir(s)).length === 0) await fsp.rm(s, { recursive: true, force: true }) } catch {} continue }
        } catch {}
        continue
      }
    ...
    let totalBytes = 0
    for (const it of toCopy) totalBytes += await getDirSizeAsync(it.s)
    let copied = 0
    for (const it of toCopy) {
      try {
        ...
        try { copied += await getDirSizeAsync(it.s) } catch {}
...
// ── Install-target triage (Tauri parity: classify_target) ──
// What is already in the install location? Verdicts: empty | ours_healthy |
// ours_broken_env | repo_no_env | pinokio | foreign. Pure fs — pass the parsed
// envs registry in (main process owns getActiveEnv).
// pinokioRoot: Pinokio markers (pinokio.js/pinokio.json/.pinokio) live in the
// app dir or up to two levels above (`api/<name>.git/app` layout).
function pinokioRoot(repoDir) {
  let cur = repoDir
  for (let i = 0; i < 3 && cur; i++) {
    try {
      if (fs.existsSync(path.join(cur, 'pinokio.js')) ||
          fs.existsSync(path.join(cur, 'pinokio.json')) ||
          fs.existsSync(path.join(cur, '.pinokio'))) return cur
    } catch {}
...
function classifyTarget(repoDir, envs) {
  const out = (verdict, hint, extra) => Object.assign({
    verdict, hint, repo: repoDir,
  }, extra || {})
  if (!repoDir || !fs.existsSync(repoDir)) {
    return out('empty', 'Empty folder — clean install.', { exists: false, entries: [] })
  }
  let names = []
  try { names = fs.readdirSync(repoDir) } catch { return out('empty', 'Empty folder — clean install.', { exists: false, entries: [] }) }
  if (!names.length) return out('empty', 'Empty folder — clean install.', { exists: true, entries: [] })
  // Only our own bookkeeping files means "empty" for install purposes.
  const BENIGN = ['desktop-config.json', '.uv-cache', 'envs.json']
  if (names.every(n => BENIGN.includes(n))) {
    return out('empty', 'Empty folder — clean install.', { exists: true, entries: names })
  }
  const hasWgp = fs.existsSync(path.join(repoDir, 'wgp.py'))
  const hasGit = fs.existsSync(path.join(repoDir, '.git'))
  let remote = null
  try {
    const cfg = fs.readFileSync(path.join(repoDir, '.git', 'config'), 'utf8')
    const m = cfg.split("\n").find(l => l.trimStart().startsWith('url ='))
    if (m) remote = m.trimStart().slice('url ='.length).trim()
  } catch {}
  const envStates = {}
  for (const ed of ['env_uv', 'env_venv', 'env_conda']) {
    const base = path.join(repoDir, ed)
    if (!fs.existsSync(base)) continue
    const py = process.platform === 'win32' ? path.join(base, 'Scripts', 'python.exe') : path.join(base, 'bin', 'python')
    envStates[ed] = { exists: true, healthy: fs.existsSync(py) }
  }
  const anyHealthy = Object.values(envStates).some(e => e.healthy)
  const managedActive = !!(envs && envs.active && envs.envs && envs.envs[envs.active])
  const hasConfig = fs.existsSync(path.join(repoDir, 'wgp_config.json'))
  const pinokio = pinokioRoot(repoDir)
  const staleTmp = names.filter(n => n.startsWith('.wan2gp-clone-tmp-'))
  const modelDirs = ['ckpts', 'loras', 'outputs'].filter(d => fs.existsSync(path.join(repoDir, d)))
  let verdict, hint
  if (pinokio) {
    verdict = 'pinokio'
    hint = "Reusing Pinokio's Wan2GP install directly is not possible — Pinokio owns that folder's lifecycle and environment. Install fresh into an empty folder and reuse Pinokio's model folders instead (no re-downloads, Pinokio keeps working)."
  } else if (hasWgp && anyHealthy) {
    verdict = 'ours_healthy'; hint = 'A working Wan2GP install is already here — you can reuse it instead of reinstalling.'
  } else if (hasWgp && Object.keys(envStates).length) {
    verdict = 'ours_broken_env'; hint = 'Wan2GP repo is here but its Python environment is broken/incomplete — repair keeps your models and settings.'
  } else if (hasWgp) {
    verdict = 'repo_no_env'; hint = 'Wan2GP repo without a Python environment — install just the environment, no re-clone needed.'
  } else {
    verdict = 'foreign'; hint = "Folder isn't empty and isn't a Wan2GP install — pick an empty folder or wipe it first so upstream files can't collide."
  }
  return out(verdict, hint, {
    entries: names.slice(0, 12), entryCount: names.length,
    hasRepo: hasWgp, hasGit, gitRemote: remote, envs: envStates,
    managedActive, hasConfig, pinokio: !!pinokio, pinokioRoot: pinokio,
    staleCloneTmp: staleTmp, modelDirs,
  })
}

module.exports = { getDirSize, getDirSizeAsync, mergeDirContents, mergeDirContentsAsync, flattenRepo, rewriteModelPaths, reconcileModelFolders, reconcileModelFoldersAsync, ensureRepoGit, cleanupLegacyRuntime, pinokioRoot, classifyTarget }
